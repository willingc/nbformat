...
from .nbbase import nbformat, nbformat_minor, NotebookNode
...
def upgrade(nb, from_version=4, from_minor=0):
    ...
    if from_version == 4:
        ...
        raise ValueError(
            "Cannot convert a notebook directly from v%s to v5.  "
...
def upgrade_cell(cell):
    ...
    if cell.cell_type == "code":
        ...
        cell.source = u"{hashes} {single_line}".format(
            hashes="#" * level, single_line=" ".join(cell.get("source", "").splitlines())
        )
    elif cell.cell_type == "rich":
        cell.pop("language", "")
        if "collapsed" in cell:
            cell.metadata["collapsed"] = cell.pop("collapsed")
        cell.source = cell.pop("input", "")
